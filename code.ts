...
async function applyProportionalResize(node: SceneNode, scaleX: number, scaleY: number, log: string[], options: { useAdaptiveText: boolean; useContentAwareImages: boolean; useAdaptiveStrokes: boolean; }) {
...
async function resizeRecursively(node: SceneNode, scaleX: number, scaleY: number, log: string[], options: { useAdaptiveText: boolean; useContentAwareImages: boolean; useAdaptiveStrokes: boolean; }) {
  if ("children" in node) {
    for (const child of node.children) {
      await applyProportionalResize(child, scaleX, scaleY, log, options);
      await resizeRecursively(child, scaleX, scaleY, log, options);
    }
  }
}
...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === 'resize-frame') {
    ...
    if (selection.length === 1 && selection[0].type === "FRAME") {
      ...
      await resizeRecursively(newFrame, scaleX, scaleY, resizeLog, resizeOptions);
